...
void sched_switch(void)
{
        ...
        kthread_t *new_kt, *run_kt;

        run_kt = ktqueue_dequeue(&kt_runq);
        context_t *curr_ctxt = &curthr->kt_ctx;
        context_t *run_ctxt = &run_kt->kt_ctx;

        curthr = run_kt;
        curproc = run_kt->kt_proc;

        context_switch(curr_ctxt, run_ctxt);
