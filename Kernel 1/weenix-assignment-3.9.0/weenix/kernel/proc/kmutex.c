...
void kmutex_lock(kmutex_t *mtx)
{
        ...
        {
                // some other thread is holding the mutex, sleep on the wait queue
                dbg(DBG_PRINT, "(GRADING1C)\n");
...
int kmutex_lock_cancellable(kmutex_t *mtx)
{
        ...
        dbg(DBG_PRINT, "(GRADING1C)\n");
        // check if current thread holds mutex
        if (curthr->kt_cancelled != 1)
        {
                ...
                {
                        dbg(DBG_PRINT, "(GRADING1C)\n");
                        int sleepreturn = sched_cancellable_sleep_on(&mtx->km_waitq);
                        if (sleepreturn == -EINTR)
                        {
                                dbg(DBG_PRINT, "(GRADING1C)\n");
                                kmutex_unlock(mtx);
                                return -EINTR;
...
void kmutex_unlock(kmutex_t *mtx)
{
        ...
        {
                // wake up the thread at the head of the wait queue
                kthread_t *thread = sched_wakeup_on(&mtx->km_waitq);
                mtx->km_holder = thread;
                dbg(DBG_PRINT, "(GRADING1C)\n");
        }
        else
        {
                // no threads waiting on mutex, set holder to NULL
                mtx->km_holder = NULL;
                dbg(DBG_PRINT, "(GRADING1C)\n");
        }
