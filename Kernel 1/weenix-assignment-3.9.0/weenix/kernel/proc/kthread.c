...
kthread_create(struct proc *p, kthread_func_t func, long arg1, void *arg2)
{
        ...
        dbg(DBG_PRINT, "(before allocstack)\n");
        // char *bstack = alloc_stack();
        dbg(DBG_PRINT, "(after allocstack)\n");
        new_thread->kt_proc = p;

        new_thread->kt_kstack = alloc_stack();

        // Not able to set it to kt_runq
        ...
        dbg(DBG_PRINT, "(before pagedir)\n");
        context_setup(&new_thread_context, func, arg1, arg2, new_thread->kt_kstack, DEFAULT_STACK_SIZE, p->p_pagedir);
        dbg(DBG_PRINT, "(after pagedir)\n");
        new_thread->kt_ctx = new_thread_context;

        // NOT_YET_IMPLEMENTED("PROCS: kthread_create");
        dbg(DBG_PRINT, "(before returning from kthread_create)\n");
        return new_thread;
}
...
void kthread_cancel(kthread_t *kthr, void *retval)
{
        ...
        if (curthr != kthr)
        {
                if (kthr->kt_state == KT_SLEEP || kthr->kt_state == KT_SLEEP_CANCELLABLE || kthr->kt_state == KT_RUN)
                {
                        sched_cancel(kthr);
                        dbg(DBG_PRINT, "(GRADING1C)\n");
                }
